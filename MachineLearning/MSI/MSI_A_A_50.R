...
#   Machine Learning                   #
#   Traing on batch A                  #
#   and Testing on A over 50 iteration #
########################################
# Clear workspace
rm(list=ls())
...
for (i in 1:50) {
  trainN <- MSI_TVC_A[train.index[,i],]
  testN <- MSI_TVC_A[-train.index[,i],]
  RF.model.fit.TVC <- train(TVC ~ ., method='rf', trcontrol = control, trainN,tuneGrid=expand.grid(mtry=1:20))
...
plot(predicted.SVMP,testN$TVC,xlim= c(0,9), ylim=c(0,9),xlab="Estimated microbial Population (log CFU/g)",ylab="Observed microbial Population (log CFU/g)", col="blue",
     main=paste("Polynomial support-vector machine Model for Total Viable Counts (TVC) \nRMSE",round(RMSE.SVMP,digits = 2)))
abline(a=0,b=1)
abline(a=-1,b=1)
abline(a=1,b=1)
###################################################################################
...
for (i in 1:50){
  trainNP <- MSI_PS_A[train.indexs[,i],]
  testNP <- MSI_PS_A[-train.indexs[,i],]
  RF.model.fit.Ps <- train(Pseudomonas.spp. ~ ., method='rf', trainNP, trcontrol = control, tuneGrid=expand.grid(mtry=1:20))
...
for (i in 1:50){
  trainNP <- MSI_PS_A[train.indexs[,i],]
  testNP <- MSI_PS_A[-train.indexs[,i],]
  SVMLM.model.fit.PS <- train(Pseudomonas.spp. ~ ., method='svmLinear', data= trainNP, trcontrol=control)
...
for (i in 1:50){
  trainNP <- MSI_PS_A[train.indexs[,i],]
  testNP <- MSI_PS_A[-train.indexs[,i],]
  knn.model.fit.ps <- train(Pseudomonas.spp. ~ ., method='knn', data=trainNP, trcontrol = control, tuneGrid=expand.grid(k=1:20))
